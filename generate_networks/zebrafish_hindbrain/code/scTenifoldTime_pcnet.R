...
dta <- readRDS("generate_networks/zebrafish_hindbrain/data/dta.rds")
sudotime <- readRDS("generate_networks/zebrafish_hindbrain/data/sudotime.rds")
...
res <- scTenifoldTime(dta_list = dta_list, method = "pcnet", time_vec = time_vec,
                      nComp = 5, q = 0, K = 5)
saveRDS(res, paste0("generate_networks/zebrafish_hindbrain/results/networks/res_pcnet_ngroup_", n_group, ".rds"))
